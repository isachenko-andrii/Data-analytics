![Data analytics](https://github.com/isachenko-andrii/Data-analytics/raw/main/img/book-cover-smoll.png)  

##  

## Зміст  
  
    <div class="table-container">
        <table class="github-table">
            <thead>
                <tr>
                    <th>ID</th>
                    <th>ПІБ Студента</th>
                    <th>Дисципліна</th>
                    <th>Оцінка (ECTS)</th>
                </tr>
            </thead>
            <tbody>
                <tr>
                    <td>1</td>
                    <td>Іваненко Іван</td>
                    <td>Data Analytics</td>
                    <td>A (Відмінно)</td>
                </tr>
                <tr>
                    <td>2</td>
                    <td>Петренко Петро</td>
                    <td>Data Analytics</td>
                    <td>B (Дуже добре)</td>
                </tr>
                <tr>
                    <td>3</td>
                    <td>Сидоренко Сидір</td>
                    <td>Економетрика</td>
                    <td>C (Добре)</td>
                </tr>
                <tr>
                    <td>4</td>
                    <td>Коваленко Анна</td>
                    <td>Бази даних (SQL)</td>
                    <td>A (Відмінно)</td>
                </tr>
                <tr>
                    <td>5</td>
                    <td>Ткаченко Марія</td>
                    <td>Data Analytics</td>
                    <td>D (Задовільно)</td>
                </tr>
                <tr>
                    <td>6</td>
                    <td>Шевченко Олег</td>
                    <td>Машинне навчання</td>
                    <td>B (Дуже добре)</td>
                </tr>
                <tr>
                    <td>7</td>
                    <td>Бондар Олена</td>
                    <td>Економетрика</td>
                    <td>FX (Незадовільно)</td>
                </tr>
                <tr>
                    <td>8</td>
                    <td>Мороз Дмитро</td>
                    <td>Бази даних (SQL)</td>
                    <td>C (Добре)</td>
                </tr>
                <tr>
                    <td>9</td>
                    <td>Кравченко Юлія</td>
                    <td>Data Analytics</td>
                    <td>A (Відмінно)</td>
                </tr>
            </tbody>
        </table>
    </div>


## Контакти  
    
**Ім'я:** [Andrii Isachenko](https://isachenko-andrii.github.io)    
**LinkedIn:** [Andrii Isachenko](https://www.linkedin.com/in/isachenko-andrii/)  
**E-mail:** andrii.isachenko@gmail.com   
